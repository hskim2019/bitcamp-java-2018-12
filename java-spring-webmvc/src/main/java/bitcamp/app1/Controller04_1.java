...
public class Controller04_1 {
	...
	// 요청 핸들러에서 받을 수 있는 타입의 아규먼트를 선언 해 보자
	@GetMapping("h1")
	@ResponseBody
	public void handler1(
		 //ServletContext sc, // ServletContext는 파라미터로 받을 수 없다. 예외 발생
			...
			HttpSession session,
			Map<String, Object> map,      // JSP 에 전달할 값을 담는 임시 보관소
			Model model,         // Map 과 같다. 둘 중 한 개만 받으면 된다
			PrintWriter out // 클라이언트에게 콘텐츠를 보낼 때 사용할 출력 스트림
			) {
