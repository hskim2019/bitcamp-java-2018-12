...
public class Controller03_2 {
	...
	public String handler1(Model model) {
		...
		// Model 객체에 값을 담으면 프론트 컨트롤러는 ServletRequest 보관소에 값을 옮긴다.(jsp가 쓸 수 있도록)
		// 만약 @SessionAttributes에서 지정한 이름의 값이라면
		// (Servlet Request뿐만 아니라) HttpSession 객체에도 보관된다
	...
	public String handler3(
			...
			// => 세션에 해당 값이 있다면 아규먼트에 넣어 준다
			// => 세션에 해당 값이 없으면 에외가 발생한다
			...
			// => 만약에 요청 파라미터에 tel2 값이 있으면 그 값을 넣어준다
			// => @ModelAttribute 는 @SessionAttributes에 있는 것을 꺼낼 때 사용하는 것
			//    그러나 name2와 age2는 modelattribute에 지정 된 값=session에 보관된 값을 꺼내는 것이므로 
			//    아규먼트에 새로운 값 넣어도 그 값이 넣어지지 않는다
			// http://localhost:8080/java-spring-webmvc/app2/c03_2/h3?tel2=1111&name2=홍길동
	...
	// 테스트:
	// http://localhost:8080/java-spring-webmvc/app2/c03_1/h1 을 실행하여 name, age 값을 세션에 보관한다
	// http://localhost:8080/java-spring-webmvc/app2/c03_2/h1 을 실행하여 name2, age2 값을 세션에 보관한다
 	// http://localhost:8080/java-spring-webmvc/app2/c03_2/h4 을 실행하여 값을 꺼낸다
	@GetMapping(value="h4", produces="text/plain;charset=UTF-8")
	@ResponseBody
	public String handler4(
			// c03_1/h1 에서 세션에 저장한 값 꺼내기
			// => 꺼내지 못한다. name, age에는 빈 문자열이 저장된다
			// => 왜?
			//    @ModelAttribute 는 현재 페이지 컨트롤러의 @SessionAttributes에
			//     지정된 이름에 대해서만 세션에서 값을 꺼내기 때문이다, session 에서 꺼내는 것이 아니라
