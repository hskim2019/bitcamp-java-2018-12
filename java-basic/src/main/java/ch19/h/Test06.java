// Lambda 문법 : functional interface 의 자격
// => 문법
//         (파라미터, 파라미터, ...) -> 문장 한개
...
public class Test06 {
  ...
  static interface Player2 {
    ...
    // 인터페이스는 default method를 가질 수 있다
    // => 용도: 기존 프로젝트에 영향을 끼치지 않으면서 기존 규칙에 새 메서드를 추가할 때 사용한다
    default void stop() {
      // 어차피 새 메서드는 새 프로젝트의 구현체가 오버라이딩(정의)할 것이니 여기에서는 정의하지 않는다
  ...
  public static void main(String[] args) {
    ...
    // static 메서드나 default 메서드가 몇 개이든 개수는 중요하지 않다
    // 단 추상 메서드가 한 개이어야 한다
