...
public class Score3 implements Serializable{
  ...
  //    이 클래스에 tel 필드와 setter/getter추가하고 toString() 다시 생성한다
  ...
  //    tel 필드는 추가 되었다 하더라도 기존의 성적 데이터에 영향을 끼치지 않기 때문에
  //    이전 버전의 값을 그대로 읽어도 된다
  //    이런 경우에 serialVersionUID 의 값을 그대로 두는 것이다
  private static final long serialVersionUID = 100L; // L = long값
  private String name;
  private String tel; // Test3_1을 실행 후 이 필드를 추가
  private int kor;
  private int eng;
  private int math;
  private int sum;
