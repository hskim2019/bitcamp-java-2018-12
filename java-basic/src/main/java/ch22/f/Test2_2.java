...
public class Test2_2 {
  public static void main(String[] args) {
    ...
    //    바이트 배열을 읽어 인스턴스 필드에 그대로 저장한ㄷ
    //    생성자는 호출하지 않는다
    // => 저장할 때 사용한 클래스가 변경된 경우에는
    //    deserialize 할 때 예외가 발생한다
    //    예) Score2 인스턴스를 저장한 후,
    //        Score2 클래스를 변경하라
    //        예를 들어 인스턴스 필드를 추가해보라, 그 후 다시 읽기 시도하면 오류 발생
    ...
    try(ObjectInputStream in = new ObjectInputStream(
      ...
      // => serialVersionUID 값을 개발자가 지정하지 않으면 컴파일 할 때 자동으로 부여된다
      // => Score2 클래스 또한 이 변수가 존재하고, score2.data 파일로 인스턴스를 출력할 때 이 변수의 값이 함께 저장된다
      // => Score2 클래스를 변경하게 되면 이 변수의 값이 자동으로 새로 부여된다
      // => ObjectInputStrea.readObject() 는 
      //    score2.data 파일을 읽을 때 함께 저장된 serialVersionUID 값과
      //    deserialize 할 때 사용할 Score2 클래스의 serialVersionUID 값을 비교하여
      //    같다면 파일의 데이터를 읽어 인스턴스를 생성하여 리턴하고
      //    다르면 예외를 발생시킨다
