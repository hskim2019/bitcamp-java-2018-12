// static nested class 문법 적용 - Node 클래스를 스테틱 중첩 클래스로 선언한다
...
public class LinkedList {
  ...
  public Object[] toArray() {
    ...
    Node cursor = head; // cursor가 가리키는 값을 찾아야 함. head부터 시작
    int i = 0;
  ...
  // LinkedList 내부에서만 사용하기 때문에 Node의 멤버를 public으로 공개할 필요 없이 default로 두기
    Object value;
    Node prev; // 이전 Node 주소 담는 필드
    Node next; // 다음 Node 주소 담는 필드
