...
public class Test08 {
  ...
  public static void main(String[] args) {
    ...
    // => key 는 값을 저장할 위치를 계산할 때 사용한다
    // => map.put(key, value);
    
    // put(Object key, Object value)
    // => key 값으로 int를 넘겨준다?    
    //       : Integer.valueOf(100) 
    //          내부적으로 오토박싱을 수행해서 Integer 객체를 만듦, 그리고 primitive 값이 아닌 객체를 넘겨줌
    // => put() 메서드는 key로 넘겨받은 객체에 대해 hashCode()를 호출하여 정수 값을 얻는다
    // => 해시 코드를 사용하여 값을 저장할 위치를 계산한ㄷ
    // => 그런 후 그 위치에 해당하는 배열(배열로 관리 한다면)에 저장한다
    ...
    map.put(Integer.valueOf(k1), new Student("홍길동", 20, false));
...
// key를 사용하여 값을 꺼내보자
    ...
    //    원래 키와 내용물이 같은지 equals()로 한번 더 비교한다
    //    만약 같다면 같은 key로 간주하여 해당 값을 꺼내 리턴한다
    
    // 결론! -> k2와 k6는 다른 객체지만 내용물은 같다
    // 따라서 k2로 저장한 값을 k6로 꺼낼 수 있다
    // 왜? 객체는 다르지만 내부 값이 같기 때문에 같은 key라고 간주한다
    //     hashCode() 의 리턴 값이 같고, equals()의 리턴 값이 true이기 때문에 같은 key로 간주한다
