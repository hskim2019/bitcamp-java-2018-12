...
public class Test16 {
  ...
  // => Cloneable 을 구현하는 이유는 JVM에게 이 클래스의 인스턴스를 복제할 수 있음을 표시하기 위함이다
  //    이 표시가 안 된 클래스는 JVM이 인스턴스를 복제 해 주지 않는다
  
  // 인스턴스를 복제할 때는
  // => Object에서 상속 받은 clone()을 호출해야 한다
  // => 그런데 clone()의 접근 제어가 protected라서
  //    같은 패키지의 멤버이거나 서브 클래스만이 호출 할 수 있다
  static class Score implements Cloneable {
    ...
    // public으로 접근 범위를 넓혀라
    // => 오버라이딩은 접근 범위를 좁힐 수 는 없지만, 넓힐 수는 있다
