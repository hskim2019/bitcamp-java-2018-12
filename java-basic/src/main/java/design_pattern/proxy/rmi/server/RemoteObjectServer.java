...
//    }
    ...
    try {
      ...
      //    => 첫 번째 파라미터 : 원격 객체
      //    =>  두 번째 파라미터 : 포트 번호, 0으로 지정하면 임의로 유효한 포트 번호가 지정된다.
      // => 이렇게 RMI 기술을 사용하면 개발자가 skeleton(서버 측 ORB)나 stub(클라이언트 측 ORB)을 작성 할 필요가 없다
      ...
      //                    만약 그 이름으로 등록된 스텁이 있다면 기존에 등록된 스텁 객체를 리턴한다.
      // => bind() : 해당 이름으로 스텁 객체를 등록한다
      //                   만약 그 이름으로 등록 된 스텁이 있다면 예외가 발생한다
      registry.rebind("calc", stub);
      
      System.out.println("Calculator 객체의 프록시(stub) RMI Registry 서버에 등록되었다.");
