...
public class Test01 {
  ...
  public static void main(String[] args) {
    B obj = new B();
    // B 클래스의 인스턴스 생성 과정
    // 1) B의 수퍼 클래스(A 클래스)를 로딩한다(로딩 되어 있지 않다면)
    //    : 스테틱 필드 생성 => 스테틱 블록 실행
    ...
    // 메서드 호출 과정
    obj.m1();  // B 클래스에서 m1() 찾는다 => A 클래스에서 m1()을 찾는다
               // 메서드를 호출할 때 마다 수퍼 클래스를 따라 올라 가면서 찾기 때문에
               // 객체지향 프로그래밍이 C와 같은 언어보다 속도가 느리다고 하는 것이다
               // A 클래스의  System.out.printf("A.v1 = %d\n", this.v1);
               // this.v1 은 B 클래스 주소를 가리킴
    obj.m2();
