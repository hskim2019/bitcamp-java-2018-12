...
public class AppConfig {
	...
	//    만약 Bean 애노테이셔넹 이름을 지정하지 않으면 메서드 이름을 사용하여 객체를 보관한다

	// DB 커넥션풀 기능을 수행할 DataSource 객체를 생성하는 메서드 정의
	@Bean
	...
	public SqlSessionFactory sqlSessionFactory(DataSource dataSource, ApplicationContext appCtx) throws Exception {
		...
		// 도메인 클래스의 별명을 자동으로 생성하도록 도메일 클래스가 들어 있는 패키지를 지정한다
		factoryBean.setTypeAliasesPackage("com.eomcs.lms.domain");
		
		// SQL 매퍼 파일이 있는 패키지를 등록한다
		factoryBean.setMapperLocations(appCtx.getResources("classpath:/com/eomcs/lms/mapper/*Mapper.xml"));
		
		return factoryBean.getObject(); // SqlSessionFactory return 함
	}
	
	
	
	// DAO 구현체 자동 생성하는 방법2:
	// => MapperScannerConfigure 객체를 통해 DAO 구현체 자동 생성
//	@Bean
...
//	}
	...
	// DAO 구현체 자동 생성하는 방법1:
	// => mybatis 에서 Spring과 함께 사용하라고 제공해주는 SqlSessionTemplate을 사용한다
