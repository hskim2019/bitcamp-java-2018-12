...
// => 기존의 SqlSessionFactory를 사용하면 무조건 SqlSession객체를 생성하는데
//    이렇게 Proxy를 만들면, SqlSession을 Thread에 보관해서 공통으로 사용하고 매번 생성하지 않는다
public class SqlSessionFactoryProxy implements SqlSessionFactory {
	...
	// 트랜잭션에 속한 상태일때는 close()를 무시하고,
	// 트랜잭션에 속한 상태가 아닐 때는 close()를 수행한다
	// sqlSession 보관 할 ThreadLocal 보관함
	...
	public SqlSessionFactoryProxy(SqlSessionFactory original) {
		...
		// 외부에서 요청이 들어오면 이 객체를 사용하여 SqlSession 객체를 만들어준다
		this.original = original;
	}

	// 트랜잭션 관리자가 트랜잭션을 시작시키면
	// 기본적으로 auto commit이 false인 SqlSession객체를 미리 만들어 스레드에 보관한다
	// beginTransaction 호출 안 하면 openSession()만 호출 되어 original SqlSession이 리턴 되고 자동으로 close 됨
	// beginTrasaction 호출되면 SqlSession이 아닌 SqlsessionProxy가 리턴 되어 트랜잭션 완료 할 때 까지 close가 안 됨
	public void prepareForTrnasaction() {
	...
	// 태랜잭션을 완료한 다음에 SqlSession 객체를 스레드에서 제거하기
	...
	// 스레드에 SqlSession이 보관되어 있으면 그 객체를 리턴하고 없으면 원래대로 SqlSession 객체를 만들어 리턴한다
	// 스레드에 SqlSession이 보관되는 경우는 어떤경우인가??
