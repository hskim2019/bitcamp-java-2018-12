...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(
    ...
    // 테스트 방법:
    // => http://localhost:8080/java-web/ex08/s

    // 리프레시
    // => 서버로부터 응답을 받고 내용을 출력한 후 특정 URL을 자동으로 요청 하도록 만들 수 있다
    ...
    // 위에서 벌써 클라이언트에게 응답을 했는데 어떠ㅎ게 응답헤더를 출력할 수 있나요?
    // => 잊지말자! out.println() 출력 한 것은 출력 스트림 버퍼에 보관되어 있다
    //    따라서 아직 클라이언트에게 응답한 상태가 아니다
    //    그래서 다음과 같이 출력을 한 후에 응답 헤더 값을 추가하거나 변경할 수 있는 것이다
    //    메서드 호출이 완료될 때 비로소 클라이언트로 응답 헤더와 
    //    버퍼에 저장된 message-body 가 출력된다
    ...
    for (int i = 0; i < 200; i++) {
      // 50바이트씩 100번 출력하면 아직 버퍼에 차지 않았기 때문에
      // 클라이언트로 출력되지 않는다. 따라서 반복문 아래에 있는 응답 헤더 설정이 유효하다
      // 그러나 200번 출력하면 8KB 버퍼가 꽉 차기 때문에
      // 반복문 다음에 헤더를 설정하기 전에 이미 버퍼 내용이 출력 된다
      // 즉 응답이 완료된다
      // 응답을 완료한 다음에 응답 헤더의 값을 변경하거나 추가해봐야 소용 없다
      out.println(i + "  ====> 1234567890123456789012345678901234567890");
    }
    
    response.setHeader("Refresh", "3;url=s100");  //3초 후에 url이후의 주소로 보냄
  }
}
