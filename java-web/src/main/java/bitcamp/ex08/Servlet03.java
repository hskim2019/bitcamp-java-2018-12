...
public class Servlet03 extends HttpServlet {
  ...
  protected void service(
    ...
    // => 사용 예:
    //    로그인 결과를 출력하지 않고 즉시 메인 화면으로 보내고 싶을 때
...
Content-Length: 0
Date: Tue, 02 Apr 2019 03:38:46 GMT
빈줄
<----콘텐츠를 보내지 않는다 즉 message-body 가 없다-->
     */
    ...
    // 버퍼가 꽉 차서 자동으로 출력되어 리다이렉트가 안먹히는 상황을 보고싶다면
    // 다음 반복문 추가
    ...
    // forward/include와 달리 '/'는 컨텍스트 루트(웹애플리케이션 루트)가 아닌 웹 서버 루트를 의미한다
    response.sendRedirect("s100");
    
    // 리다이렉트를 하는 순간 이전까지 버퍼로 출력된 내용은 모두 버려진다
    // 왜? 리다이렉트는 클라이언트로 콘텐트를 보내지 않는다
    
    // 만약 출력한 내용이 버퍼를 꽉 채워서 자동으로 응답을 했다면 어떻게 되나?
