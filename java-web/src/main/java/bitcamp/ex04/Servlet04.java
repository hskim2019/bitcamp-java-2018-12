...
public class Servlet04 extends GenericServlet {
  ...
  public void service(ServletRequest req, ServletResponse res)
    ...
    // 멀티파트 형식의 데이터 처리하기
    // 1) Apache 라이브러리 가져온다
    //     - mvnrepository.com 에서 apache fileupload 라이브러리 검색한다
    //     - build.gradle에 라이브러리 정보 추가
    ...
    try {
      ...
      // Fileitem 인터페이스: multipart/form-data 형식으로 전송된 폼 데이터를 파일 또는 폼 아이템으로 표현하는 객체
      ...
      for (FileItem part : parts) {
        if(part.isFormField()) {
          ...
          // 업로드 파일을 젖아할 때 사용할 파일명을 준비한다
          // => 원래의 파일명을 사용하지 않는다
          // => 다른 클라이언트가 같은 이름의 파일을 업로드하면 기존 파일을 덮어쓸 수 있기 때문이다
