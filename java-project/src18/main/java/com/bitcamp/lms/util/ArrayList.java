...
public class ArrayList<E> {
  ...
  public void add(E value) {
    ...
    list[this.size++] = value; // ex) size =5 이면 arr[5] = value, 이후 size +1 증가
  }
  ...
  public E get(int index) {
    ...
    if(index<0 || index >=size)
      return null;
    return (E) this.list[index];  // 찾는 방 번호가 유효하면 방 번호 전달
  }
  ...
  public E set(int index, E value) {
    ...
    @SuppressWarnings("unchecked")
    E old = (E) this.list[index];  //원래 값 넣기
    this.list[index] = value;       // 새로운 값 넣기
    return (E) old;                    //
  }
  ...
  private void increase() {
    ...
    for (int i = 0; i < this.list.length; i++) {  // 새로 생성 된 배열에 옛날 값 옮기기
      temp[i] = this.list[i];                   
    }
    list = temp;   //  새 배열 주소 arr에 저장하고, 이전 것은 가비지가 됨
  }
}
