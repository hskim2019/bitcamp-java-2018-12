// List(interface) 사용 규칙에 따라 LinkedList를 만든다
...
public class LinkedList<E> implements Cloneable, List<E> {
  ...
  public <T> T[] toArray(T[] a) {
    // 파라미터로 받은 배열의 크기가 리스트의 모든 항목을 담을 만큼 크다면 배열을 새로 만들지 않고 그대로 사용하겠다
    T[] arr = null;
    if(a.length >= size()) {
      arr = a;
    } else { // 만약 파라미터로 받은 배열의 크기가 더 크면 타입을 복사해서 배열 새로 만듦
      arr = (T[]) Array.newInstance(a.getClass().getComponentType(), this.size());// 현재 사이즈 길이 만큼의 arr 배열 만듦 
      //배열의 타입은??  아규먼트 a를 준 클래스의 타입
      // 리턴 타입은 T가 아니라 T[] / getComponentType()의 리턴 값은 배열의 항목 타입. 즉 T
    }
    Node<E> cursor = head; // cursor가 가리키는 값을 찾아야 함. head부터 시작
    int i = 0;
    while(cursor != tail) { // 커서가 tail을 가리킨다는 것은 빈 노드 
